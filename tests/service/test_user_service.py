...
def test_get_all_users_exception(db_session, monkeypatch):
    ...
    with pytest.raises(user_service.UserOperationError) as e:
...
def test_create_user_duplicate_username_raises(db_session):
    user_service.create_user('dup_user', 'Test', 'User', 100.00)
    with pytest.raises(user_service.UserOperationError) as e:
        user_service.create_user('dup_user', 'Test', 'User', 100.00)
...
def test_delete_admin_user_raises(db_session):
    ...
    assert "Cannot delete admin user" in str(e.value)
...
def test_delete_user_with_dependencies_raises(db_session):
    user_service.create_user('user1', 'Test', 'Mr. User', 200.00)
...
def test_update_user_balance(db_session):
    user_service.create_user('bal_user', 'Balance', 'User', 1000.00)
    user_service.update_user_balance('bal_user', 500.00)

    user = db_session.query(User).filter_by(username='bal_user').one()
...
def test_get_user_by_username(db_session):
    user_service.create_user('lookup_user', 'Lookup', 'User', 250.00)

    user = user_service.get_user_by_username('lookup_user')
    assert user is not None
    assert user.username == 'lookup_user'
    assert user.firstname == 'Lookup'
